    ...
    def map_loops(self) -> None:
        ...
        for i, char in enumerate(code):
            ...
            if (i == len(code) - 1 and char == ")") or (char == ")" and not code[i + 1].isnumeric()):
                ...
                while sub_i < len(code) and code[sub_i].isnumeric():
                    repeat_num += code[sub_i]
                    sub_i += 1

                loop_map[loop_start] = [i, int(repeat_num)] #forward mapping for forward jumps with repeat numbers 
                loop_map[i] = [loop_start]
...
"""This solution is based on my original architecture for the RS interpreter"""
import copy
...
class Robot:
    ...
    def map_comments(self, code_line: str) -> dict[int, int]:
        line: str = code_line
        stack: list[int] = []
        ...
        while lp < len(line):
            ...
            if char == "/" and next_char == "*":
                ...
                comment_start: int = stack.pop()
                com_map[comment_start] = next_i #forward mapping
    ...
    def lexer(self) -> None:
        ...
        while lp < len(code_lst):
            ...
            if len(line) == 0:
                ...
                if line.find("*/") != -1:
                    comment_map: dict[int, int] = self.map_comments(code_line = line)

                    char_p: int = 0
                    while char_p < len(line):
                        if char_p in comment_map.keys():
                            char_p = comment_map[char_p]

                            if char_p + 1 < len(line) and line[char_p + 1].isnumeric():
                                     raise SyntaxError(f"tokenizer: at position {line[char_p + 1]} a comment should not be followed by a numeric value.")

                        else:
                            clean_line += line[char_p]
                ...
                pass_line: bool = False
                if clean_line.startswith("//"):
                    pass_line = True
                    pass

                elif clean_line.find("//"):
                    ...
                    clean_line = clean_line.split("//", 2)[0]

                if len(clean_line) > 0 and pass_line == False:
                    clean_code_lst.append(clean_line)

            elif line.find("*/") != -1:
                comment_end: int = 0
                
                if line.find("*/") == len(line) - 2: #ml-comment encompasses the whole line
                    if len(next_line) != 0 and next_line[0].isnumeric():
                        raise SyntaxError(f"tokenizer: at position {lp} a comment should not be followed by a numeric value.")
    ...
    def parser(self, token_list: list[Token], instruction_list: list[Instruction], start_index: int = 0, in_pattern: bool = False) -> None:
        ...
        while lp < len(tokens):
            ...
            if token.type == "instruction":
                ...
                if tokens[loop_end + 1].type == "repeat":
                    loop_repeat: int = int(tokens[loop_end + 1].value)
    ...
    def execute(self, instruction_list: list[Instruction], pattern_list: list[Pattern], start_index: int = 0, sub_process: bool = False, return_env: list[str] = ["global"]) -> None:
        instructions: list[Instruction] = copy.deepcopy(instruction_list)
        ...
        while ip < len(instructions):
            ...
            if inst.type == "instruction":
                ...
                #recursive self call to execute in-pattern sub-instructions
                local_return_env: list[str] = self.env_stack
                sub_instructions: list[Instruction] = pattern_lst[call_index].value
                self.execute(instruction_list = sub_instructions, pattern_list = self.pattern_lst, start_index = 0, sub_process = True, return_env = local_return_env)
...
"""This solution follows the instruction given by the kata author"""
import re
import copy
from typing import TypedDict
...
class RSUProgram:
    ...
    def replace_pattern_call(self, input_instruction_list: list[str], output_instruction_list: list[str], return_env: list[str] = ["global"]) -> None:
        ...
        while lp < len(inst_lst):
            ...
            if re.match(pattern = "[P][0-9]+", string = token):
                ...
                self.replace_pattern_call(input_instruction_list = sub_instructions, output_instruction_list = output_instruction_list, return_env = local_return_env)
    ...
    def execute_raw(self, instructions: list[str]) -> str:
        commands: list[str] = copy.deepcopy(instructions)
        ...
        while inst_pointer < len(commands):
            ...
            elif commands[inst_pointer] == "L" or commands[inst_pointer] == "R":
                self.turn(direction = commands[inst_pointer])
    ...
    def execute(self) -> str:
        self.get_tokens()
        self.convert_to_raw(self.token_lst)
        self.execute_raw(self.output_list)
